...
export default function StatusPage() {
    ...
    const fetchStatus = useCallback(async () => {
        ...
        try {
            ...
            const statusResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/form-reviews/${formId}`);
            if (!statusResponse.ok) {
                throw new Error(`Failed to fetch form reviews status: ${statusResponse.status}`);
            }
            ...
            if (statusResult.status === "approved") {
                ...
                const dateResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/auth/form/${id}`);
                if (!dateResponse.ok) {
                    throw new Error(`Failed to fetch form scheduled date: ${dateResponse.status}`);
                }
            ...
            {status === "approved" ? (
                <div className="text-center bg-white dark:bg-gray-600 text-green-400 p-6 rounded-md shadow-md">
                    ...
                    {isTodayScheduledDate ? (
                        ...
                        <p>The link will be available on {scheduledDate ? format(scheduledDate, "MMM dd, yyyy") : "the scheduled date"}.</p>
                    )}
                </div>
            ) : status === "pending" ? (
                <div>
